...
- **Styly:** Tailwind 3 + `tailwindcss-animate` + `@tailwindcss/typography`, custom CSS vars v [src/index.css](src/index.css), barvy a fonty v [tailwind.config.ts](tailwind.config.ts). Fontových rodin je **pět**, všechny se načítají třemi Google Fonts `@import` na [src/index.css:1-3](src/index.css): `Montserrat` (`font-display`), `Inter` (`font-sans`), `League Spartan` (`font-spartan`), `Space Grotesk` (`font-grotesk`) a `IBM Plex Mono` (`font-plex`). Poslední dvě patří dodavatelské větvi BigDealSupplier. Balíčky `@fontsource/*` v `package.json` jsou **nepoužité** — nic je neimportuje, fonty jedou z Google Fonts.
- **State management:**
...
- **Testy:** Vitest + Testing Library (jsdom) — [vitest.config.ts](vitest.config.ts); E2E přes Playwright — [playwright.config.ts](playwright.config.ts), skript `npm run test:e2e`, testy patří do `e2e/`. **Pozn.:** config původně importoval Lovable preset `lovable-agent-playwright-config`, jenže ten balík na veřejném npm neexistuje (404) a není v `package.json` — config se tím pádem nedal ani načíst. 2026-08-07 nahrazen standardní konfigurací nad `@playwright/test`.
- **Lint:** ESLint 9 flat config — [eslint.config.js](eslint.config.js)
- **Dvě značky v jednom repu:** odběratelská **swelt.partner** (velkoobchod a dropshipping pro odběratele) a dodavatelská **BigDealSupplier** na `/suppliers` ([src/pages/Suppliers.tsx](src/pages/Suppliers.tsx)) — landing, kde značky a distributoři nabízejí closeout/overstock zboží. Mají oddělený vizuální jazyk (Space Grotesk + IBM Plex Mono) i tone of voice: skilly v [.claude/skills](.claude/skills) — `swelt-copywriting` pro odběratele, `bigdealsupplier-copywriting` pro dodavatele. **Pravidla jsou záměrně obrácená, nezaměňovat je.**
...
```
/                       — kořen
├─ api/                 — Vercel serverless funkce (7 endpointů, ne jen chat):
│  ├─ chat.ts           — AI asistent „Věra" (Anthropic SDK)
│  ├─ context.ts        — context bundle pro admin Context Hub
│  ├─ generate-offer.ts — generování nabídky
│  ├─ mcp.ts            — VLASTNÍ MCP server `swelt-admin-context` (streamable HTTP,
│  │                      tooly get_context + query_data, auth Bearer MCP_TOKEN;
│  │                      bez MCP_TOKEN vrací 503 = vypnuto)
│  ├─ process-inquiries.ts        — cron 07:00 UTC (vercel.json)
│  ├─ process-supplier-orders.ts  — cron 06:00 UTC (vercel.json)
│  └─ _lib/, ai/        — sdílené helpery
...
│  ├─ pages/            — stránky odpovídající routám (Index, Shop, Deals, Brands, Suppliers, …)
...
│  │  └─ suppliers/     — dodavatelská větev (SupplierGateDialog)
...
| `/suppliers`                  | `Suppliers`               | **BigDealSupplier** — dodavatelská landing (closeout/overstock). Vlastní vizuální jazyk (Space Grotesk/IBM Plex Mono) i copy skill, viz sekce 1 |
...
Globální UI prvky: `Navbar`, `BottomNav` (mobile), `CartDrawer`, `WishlistDrawer`, `ChatWidget` (AI asistent „Věra"), `SupplierGateDialog` (vstupní brána dodavatelské větve), `SalesModeBar` (admin se může „převtělit" do zákazníka přes `CustomerSelectorPanel`).
