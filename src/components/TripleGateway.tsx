...
import { KatalogGateway } from '@/components/KatalogGateway';
...
export function TripleGateway({ onOpenCatalog }: Props) {
  ...
  const handleClick = (key: string) => {
    if (key === 'dropshipping') navigate('/dropshipping');
    else if (key === 'luxury') navigate('/luxury');
  };

  return (
    <div className="w-full" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      {/* LIVE KATALOG 2026 full gateway */}
      <KatalogGateway onOpenCatalog={onOpenCatalog} />

      {/* Remaining placeholder banners */}
      <div className="px-4 py-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-5 max-w-7xl mx-auto">
