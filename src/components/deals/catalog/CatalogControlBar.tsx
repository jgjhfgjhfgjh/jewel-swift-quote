 ...
 * a zrušení filtrů. Lepí se pod horní hranu jako stavový řádek terminálu,
 * aby ovládání zůstalo po ruce i hluboko v katalogu.
 */
export function CatalogControlBar({
  ...
  return (
    <div className="sticky top-0 z-30 border-b border-white/10 bg-[#0B1215]/85 py-3 backdrop-blur supports-[backdrop-filter]:bg-[#0B1215]/70">
          ...
          {sorts.map((o) => (
            <button
              ...
              className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${
                sort === o.key ? 'bg-white text-zinc-900' : 'text-zinc-400 hover:bg-white/10 hover:text-white'
              }`}
            >
              {o.label}
            </button>
          ...
          {activeCount > 0 && (
            <>
              ...
              <button
                type="button"
                onClick={onClear}
                className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-zinc-900 transition-colors hover:bg-zinc-200"
              >
