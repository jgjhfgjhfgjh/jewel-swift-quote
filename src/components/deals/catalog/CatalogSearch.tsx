...
/**
 * Vyhledávací pole katalogu — tmavá varianta na matně černé ploše: jemně
 * prosvětlené pole s vlasovým bílým rámečkem, focus rámeček rozjasní. Dvě
 * podoby:
  ...
  const input = (
    <label className={`relative flex items-center ${compact ? '' : 'mx-auto max-w-2xl'}`}>
      ...
      <input
        ...
        className={`w-full rounded-full border border-white/10 bg-white/[0.04] text-white
                    transition-colors duration-200 placeholder:text-zinc-500
                    hover:border-white/20 focus:border-white/40 focus:outline-none ${
      ...
      {value && (
        <button
          ...
          onClick={() => onChange('')}
          aria-label={c.clear}
          className={`absolute flex h-6 w-6 items-center justify-center rounded-full border border-white/10 bg-white/10 text-zinc-300 transition-colors hover:border-white/30 hover:text-white ${
