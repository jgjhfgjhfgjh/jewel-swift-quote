      ...
      {open && (results.length > 0 || showCustom) && (
        <div className="absolute z-30 mt-2 w-full overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-2xl">
            ...
            {results.map((m, i) => {
              ...
              return (
                <li key={m.id}>
                  ...
                  >
                    <span className="flex h-10 w-12 shrink-0 items-center justify-center rounded-lg border border-zinc-100 bg-zinc-50/70 px-1.5">
                      <HouseLogo
                        name={m.brand}
                        domain={m.domain}
                        width={160}
                        height={80}
                        className="max-h-5 w-auto max-w-full object-contain [mix-blend-mode:multiply]"
            ...
            {showCustom && (
              <li className="border-t border-zinc-100">
                <button
                  ...
                  className={`flex w-full items-center gap-3 px-3 py-2.5 text-left transition ${
                    highlight === results.length ? 'bg-zinc-50' : ''
                  }`}
                >
                  <span className="flex h-10 w-12 shrink-0 items-center justify-center rounded-lg border border-dashed border-zinc-200 bg-white">
                  ...
                  <span className="inline-flex shrink-0 items-center gap-0.5 text-[11px] text-zinc-400">
          ...
          <div className="border-t border-zinc-100 bg-zinc-50/60 px-3 py-2 text-[11px] text-zinc-400">
            Nenašli jste konkrétní model? Napište referenci — dohledáme cokoliv jako na Chrono24.
          </div>
        </div>
      )}
